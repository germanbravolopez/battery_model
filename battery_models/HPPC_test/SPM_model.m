...
i_salida = -6.019;% [A] Corriente de salida de la celda
r = linspace(0,1e-4,60); % [cm] La distancia completa del electrodo
t = linspace(0,6000,800); % [s] Tiempo de simulación
tiempo = t;
...
% Molar flux equation
    global I
    I = Intensidad_inicial(t); % [A*cm^-2] Corriente por unidad de área
    
    j_np = -(I)./(F*a_p*L_p);
    j_nn = (I)./(F*a_n*L_n);

% Li-ion concentration in the solid particles
% Ejecución inicial. Búsqueda del tiempo de corte
    global t_corte
