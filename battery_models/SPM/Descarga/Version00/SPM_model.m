...
i_salida = -6.02; % [A] Corriente de salida de la celda
...
% Li-ion concentration in the solid particles
    c_sp = concentration_p_ini(r,t);
    I = i_salida/A;
    c_sn = concentration_n_ini(r,t);
    I = i_salida/A;
   
    tmin_n = find(c_sn(:,end) == min(c_sn(:,end)));
    tmin_p = find(c_sp(:,end) == max(c_sp(:,end)));
...
% --------------------------------------------------------------
% Bring C-rate off
    [x_p y_p] = find(c_sp == max(max((c_sp))));
    [x_n y_n] = find(c_sn == min(min((c_sn))));
